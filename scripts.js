...
async function getTickerData(ticker) {
  try {
    const response = await axios.get(`https://api.example.com/ticker/${ticker}`);
...
async function fetchAndDisplayData() {
  ...
  for (const ticker of TICKERS) {
    ...
    if (currentPrice <= thresholdPrice) {
      ...
      row.innerHTML = `
        <td>${ticker}</td>
        <td class="${priceCellClass}">${currentPrice}</td>
        <td>${analystTargetPrice}</td>
        <td>${priceChangePercent.toFixed(2)}%</td>
        <td>${movingAverageSlope > 0 ? '+' : '-'}</td>
        <td>${analystRecommendationScore}</td>
        <td>${currentTime}</td>
      `;
