...
async function fetchAndDisplayData() {
  ...
  console.log("Last update element:", lastUpdateElement);
  ...
  for (const ticker of sp500Tickers) {
    ...
    const {
      ...
      low52Week,
      analystTargetPrice,
      analystRecommendationScore,
      movingAverageSlope
    } = data;

    const thresholdPrice = low52Week * 1.05;
    
    console.log(`Ticker: ${ticker}, Current Price: ${currentPrice}, 52 Week Low: ${low52Week}, Threshold: ${thresholdPrice}`);

    if (currentPrice <= thresholdPrice) {
      const priceChangePercent = ((analystTargetPrice - currentPrice) / currentPrice) * 100;

      const row = document.createElement('tr');
      let priceCellClass = '';
      if (currentPrice <= low52Week) {
        priceCellClass = 'highlight-red';
      } else if (currentPrice <= thresholdPrice) {
        priceCellClass = 'highlight-orange';
      }

      row.innerHTML = `
        <td>${ticker}</td>
        <td class="${priceCellClass}">${currentPrice.toFixed(2)}</td>
        <td>${analystTargetPrice.toFixed(2)}</td>
        <td>${priceChangePercent.toFixed(2)}%</td>
        <td>${movingAverageSlope > 0 ? '+' : '-'}</td>
        <td>${analystRecommendationScore.toFixed(2)}</td>
        <td>${currentTime}</td>
      `;
